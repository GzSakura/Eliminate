...
public class CullingUtils {
    ...
    private static void updateDynamicCullingDistance(MinecraftClient client) {
        ...
        if (currentTime - lastFpsUpdateTime >= 20) { // Update every second
            // In 1.20.1, fpsDebugString format is different
            String fpsString = client.fpsDebugString;
            if (fpsString.contains("fps")) {
                // Format like "30 fps"
                fpsString = fpsString.split(" ")[0];
            } else {
                // Format like "30"
                fpsString = fpsString.split(" ")[0];
            }
            currentFps = 60.0;
            try {
                currentFps = Double.parseDouble(fpsString);
